...
const SimpananForm = () => {
  ...
  const [isLoadingRekenings, setIsLoadingRekenings] = useState(false)
  ...
  // --- Filter Pencarian Rekening ---
  useEffect(() => {
    if (rekeningSearch.trim()) {
      const filtered = rekenings.filter(rekening =>
        rekening.namarekening.toLowerCase().includes(rekeningSearch.toLowerCase()) ||
        rekening.norekening.toLowerCase().includes(rekeningSearch.toLowerCase())
      )
      setFilteredRekenings(filtered)
    } else {
      setFilteredRekenings(rekenings)
    }
  }, [rekeningSearch, rekenings])
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    {/* Dropdown Hasil Pencarian Rekening */}
                    {showRekeningDropdown && (
                      <div className="absolute z-50 w-full bg-white border border-cyan/20 rounded-lg shadow-xl mt-1 max-h-60 overflow-y-auto">
                        {isLoadingRekenings ? (
                          <div className="p-4 text-center text-sm text-gray">Memuat data...</div>
                        ) : filteredRekenings.length === 0 ? (
                          <div className="p-4 text-center text-sm text-gray">Rekening tidak ditemukan</div>
                        ) : (
                          filteredRekenings.map((rekening) => (
                            <button
                              ...
                              onClick={() => {
                                setSelectedRekening(rekening)
                                setFormData({ ...formData, rekening_id: rekening.id })
                                setShowRekeningDropdown(false)
                                setRekeningSearch('')
                              }}
                              className="w-full px-4 py-3 hover:bg-cyan/5 text-left border-b border-gray-50 last:border-0"
                            >
                              <div className="font-medium text-navy text-sm">{rekening.namarekening}</div>
                              <div className="text-xs text-gray">No. {rekening.norekening}</div>
                            </button>
                          ))
                        )}
                      </div>
                  ...
                  {selectedRekening && (
                    <div className="mt-3 p-3 bg-cyan-50 border border-cyan-100 rounded-lg flex items-center justify-between">
                      <div>
                        <div className="text-xs text-cyan-700 font-medium">Rekening Terpilih:</div>
                        <div className="text-sm font-bold text-navy">{selectedRekening.namarekening} ({selectedRekening.norekening})</div>
                      </div>
                      <button
                        type="button"
                        onClick={() => {
                          setSelectedRekening(null)
                          setFormData({ ...formData, rekening_id: null })
                        }}
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl border border-cyan/20 p-6 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-gray">Rekening</span>
                  <span className="font-medium text-navy text-right">
                    {selectedRekening
                      ? `${selectedRekening.namarekening} (${selectedRekening.norekening})`
                      : '-'}
                  </span>
                </div>
